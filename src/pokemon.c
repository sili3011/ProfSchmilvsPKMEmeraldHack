...
u16 GetEvolutionTargetSpecies(struct Pokemon *mon, u8 mode, u16 evolutionItem, struct Pokemon *tradePartner)
{
    ...
    case EVO_MODE_NORMAL:
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_LEVEL_FOG:
                j = GetCurrentWeather();
                if (gEvolutionTable[species][i].param <= level && (j == WEATHER_FOG_HORIZONTAL || j == WEATHER_FOG_DIAGONAL))
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_MAPSEC:
...
static bool8 ShouldSkipFriendshipChange(void)
{
    if (gMain.inBattle && gBattleTypeFlags & (BATTLE_TYPE_FRONTIER))
        return TRUE;
    if (!gMain.inBattle && (InBattlePike()))
        return TRUE;
    return FALSE;
}
...
{
    if (gMonFrontPicTableFemale[species].data != NULL || gMonPaletteTableFemale[species].data != NULL || gMonBackPicTableFemale[species].data != NULL || gMonShinyPaletteTableFemale[species].data != NULL || gMonIconTableFemale[species] != NULL)
        return TRUE;
